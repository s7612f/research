...
class JobManager:
    ...
    def _run(self, job, topic, hours, focus, model, provider, db_path):
        try:
            artifacts = research_agent.run(topic=topic, hours=hours, focus=focus,
                                           model=model, provider=provider, db_path=db_path)
            job['state'] = 'complete'
            job['progress'] = 1.0
    ...
    def start(self, topic: str, hours: float, focus: str, model: Optional[str]=None, provider: Optional[str]=None, db_path: Optional[str]=None) -> str:
        with self._lock:
            ...
            self.current = job
            self.run_count += 1
            t = threading.Thread(target=self._run, args=(job, topic, hours, focus, model, provider, db_path), daemon=True)
            t.start()
            return job_id
