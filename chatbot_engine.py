...
class ChatbotEngine:
    """Minimal chat engine supporting Ollama or OpenAI."""
    ...
    def chat(self, session_id, message, uncensored=False):
        ...
        reply = self._call_backend(prompt)
    ...
    def _call_backend(self, prompt):
        ...
        api_type = llm.get('api_type', 'ollama')
        if api_type == 'openai':
    ...
    def _call_openai(self, prompt):
        try:
            ...
            req = request.Request('https://api.openai.com/v1/chat/completions', data=data, headers=headers)
            with request.urlopen(req) as resp:
